binomial_cdf <- function(p, k, n) {
  cdf <- pbeta(p, k + 1, n - k, lower.tail = FALSE)
  return(cdf)
}

cdf_loss_function <- function (k, quantile) {
  return (quantile - binomial_cdf(p, k, n))^2
}

binomial_quantile <- function (quantile) {
  value <- optimize(
    f = cdf_loss_function(quantile),
    interval = c(0.01, 100)
  )
  return(value)
}
